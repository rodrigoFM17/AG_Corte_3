...
class Individuo:
    ...
    def calcular_fitness(self):
        """Calcula la aptitud del individuo basado en el área utilizada y la compacidad"""
        # Inicializar matriz de la lámina (0 = vacío, 1 = ocupado)
        matriz_lamina = np.zeros((self.lamina_alto, self.lamina_ancho))
        
        self.distribucion = []  # Reiniciar la distribución
        area_total_utilizada = 0
        piezas_ubicadas = 0
        
        # Algoritmo de colocación de piezas usando Bottom-Left
        for pieza in self.piezas:
            ubicada = False
            
            # Intentar ubicar la pieza en cada posición posible
            for y in range(self.lamina_alto - pieza.alto + 1):
                for x in range(self.lamina_ancho - pieza.ancho + 1):
                    ...
                    for i in range(pieza.alto):
                        for j in range(pieza.ancho):
                            if matriz_lamina[y + i, x + j] == 1:
                                puede_ubicar = False
                                break
                        if not puede_ubicar:
                            break
                    ...
                    if puede_ubicar:
                        ...
                        for i in range(pieza.alto):
                            for j in range(pieza.ancho):
                                matriz_lamina[y + i, x + j] = 1
                        
                        pieza_ubicada = pieza.copia()
                        pieza_ubicada.x = x
                        pieza_ubicada.y = y
                        self.distribucion.append(pieza_ubicada)
                        
                        area_total_utilizada += pieza.area()
                        piezas_ubicadas += 1
                        ubicada = True
                        break
                if ubicada:
        ...
        # Calcular el fitness (área utilizada / área total)
        area_lamina = self.lamina_ancho * self.lamina_alto
        self.fitness = area_total_utilizada / area_lamina
        
        # Penalizar si no se ubicaron todas las piezas
        if piezas_ubicadas < len(self.piezas):
            self.fitness *= piezas_ubicadas / len(self.piezas)
